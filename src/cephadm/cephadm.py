...
from cephadmlib.context import CephadmContext, DeploymentType
...
class SNMPGateway(ContainerDaemonForm):
    ...
    def uid_gid(self, ctx: CephContainer) -> Tuple[int, int]:
...
class Keepalived(DaemonForm):
    ...
    def sysctl_settings(self) -> List[str]:
        return self.get_sysctl_settings()
...
def _deploy_daemon_container(
    ...
    if isinstance(daemon, UserGroupDaemonForm):
